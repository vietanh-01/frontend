...
const AdminHistoryPay = ()=>{
    ...
    useEffect(()=>{
        const getHistoryPay= async() =>{
            var response = await getMethod('/api/history-pay/admin/all?size='+size+'&sort=id,desc&page='+0);
            var result = await response.json();
            setItems(result.content)
            setpageCount(result.totalPages)
            url = '/api/history-pay/admin/all?size='+size+'&sort=id,desc&page='
        };
        getHistoryPay();
    }, []);
    ...
    async function search() {
        ...
        var ls = '/api/history-pay/admin/all?size='+size+'&search='+search+'&sort=id,desc';
        if(from != "" && to != ""){
    ...
    const getHistoryPayUser= async(user) =>{
        var response = await getMethod('/api/history-pay/admin/find-by-user?userId='+user.id);
        var result = await response.json();
        setItemUser(result)
    };
    ...
    return (
        <>
            <div class="headerpageadmin d-flex justify-content-between align-items-center p-3 bg-light border">
                <strong class="text-left"><i className='fa fa-users'></i> Quản Lý Lịch Sử Nạp Tiền</strong>
            ...
            <div class="tablediv">
                <div class="headertable">
                    <span class="lbtable">Danh sách lịch sử nạp tiền</span>
                </div>
...
export default AdminHistoryPay;