...
const AdminUser = () => {
    ...
    useEffect(() => {
        const getUser = async () => {
            var response = await getMethod(
                "/api/user/admin/get-user-by-role?&size=" +
                size +
                "&sort=id,desc&page=" +
                0
            );
    ...
    async function lockOrUnlock(id, type) {
        ...
        const response = await postMethod(
            "/api/user/admin/lockOrUnlockUser?id=" + id
        );
                    ...
                    <table id="example" class="table table-bordered">
                        ...
                        <tbody>
                            {items.map((item) => {
                                var btn = '';
                                if (item.actived == 0) {
                                    var btn = <button onClick={() => lockOrUnlock(item.id, 0)} class="btn btn-danger"><i class="fa fa-lock"></i></button>
                                } else {
                                    var btn = <button onClick={() => lockOrUnlock(item.id, 1)} class="btn btn-primary"><i class="fa fa-unlock"></i></button>
                                }
                                return (
                                    <tr>
                                        <td>{item.id}</td>
            ...
            <div class="modal fade" id="addtk" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="false">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <h5 class="modal-title" id="exampleModalLabel">Thêm tài khoản quản trị</h5> <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
